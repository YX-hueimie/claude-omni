...
(function () {
  ...
  const translations = {
    ...
    "New chat": "新建对话",
    "Search chats": "搜索对话",
    // DOM 收割器抓到的自带 UI 控件 (aria-label / title / 导航), 排除了用户内容(会话/项目/插件名)
    ...
    "Repository and pull request controls": "仓库和拉取请求控件",
    "Ask a quick question…": "问个快速问题…",
    ...
    // 屏蔽/允许网站对话框 (标题的 "sites to block" 加粗拆节点, 整段+片段都加)
    "Add sites to block": "添加要屏蔽的网站",
    "sites to block": "要屏蔽的网站",
    "Add sites to allow": "添加要允许的网站",
    "sites to allow": "要允许的网站",
    ...
    "Human Resources": "人力资源",
    "Small Business": "小型企业",
    // === Anthropic 插件目录: 名字 (描述性名字翻译, 品牌/自造词保留英文) ===
    "Code review": "代码评审",
    ...
    "Playground": "演练场",
    "Ralph loop": "Ralph 循环",
    // === Anthropic 插件目录: 描述 (完整原文, DOM 收割所得) ===
    "Adds educational insights about implementation choices and codebase patterns (mimics the deprecated Explanatory output style)": "在实现选择和代码库模式上补充教学性洞见（模拟已弃用的 Explanatory 输出风格）",
    "Agent that simplifies and refines code for clarity, consistency, and maintainability while preserving functionality. Focuses on recently modified code.": "在保留功能的前提下，简化并打磨代码以提升清晰度、一致性和可维护性的 agent。聚焦最近修改过的代码。",
    "Analyze codebases and recommend tailored Claude Code automations such as hooks, skills, MCP servers, and subagents.": "分析代码库，推荐量身定制的 Claude Code 自动化，如 hook、skill、MCP 服务器和 subagent。",
    "Automated code review for pull requests using multiple specialized agents with confidence-based scoring to filter false positives": "用多个专门 agent 对 pull request 做自动化代码评审，以基于置信度的打分过滤误报",
    "Commands for git commit workflows including commit, push, and PR creation": "用于 git 提交工作流的命令，涵盖 commit、push 和创建 PR",
    "Comprehensive PR review agents specializing in comments, tests, error handling, type design, code quality, and code simplification": "全面的 PR 评审 agent，专精于注释、测试、错误处理、类型设计、代码质量和代码简化",
    "Comprehensive feature development workflow with specialized agents for codebase exploration, architecture design, and quality review": "全面的功能开发工作流，配备用于代码库探索、架构设计和质量评审的专门 agent",
    "Comprehensive toolkit for developing Claude Code plugins. Includes 7 expert skills covering hooks, MCP integration, commands, agents, and best practices. AI-assisted plugin creation and validation.": "开发 Claude Code 插件的全面工具包。包含 7 个专家 skill，涵盖 hook、MCP 集成、命令、agent 和最佳实践。AI 辅助插件创建与校验。",
    "Connect Claude to a private MCP server through an Anthropic MCP tunnel. The /create-docker-mcp-tunnel command drives the Docker Compose quickstart end to end: certificates, proxy config, cloudflared, and a verifiable sample server.": "通过 Anthropic MCP 隧道把 Claude 连接到私有 MCP 服务器。/create-docker-mcp-tunnel 命令端到端驱动 Docker Compose 快速上手：证书、代理配置、cloudflared，以及一个可验证的示例服务器。",
    "Create distinctive, production-grade frontend interfaces with high design quality. Generates creative, polished code that avoids generic AI aesthetics.": "创建独具特色、生产级、设计质量出众的前端界面。生成有创意、精致、避免千篇一律 AI 审美的代码。",
    "Development kit for working with the Claude Agent SDK": "用于 Claude Agent SDK 开发的工具包",
    "Easily create custom hooks to prevent unwanted behaviors by analyzing conversation patterns or from explicit instructions. Define rules via simple markdown files.": "通过分析对话模式或明确指令，轻松创建自定义 hook 以阻止不想要的行为。用简单的 markdown 文件定义规则。",
    "Interactive learning mode that requests meaningful code contributions at decision points (mimics the unshipped Learning output style)": "在决策点请求有意义代码贡献的交互式学习模式（模拟未发布的 Learning 输出风格）",
    "Interactive self-referential AI loops for iterative development, implementing the Ralph Wiggum technique. Claude works on the same task repeatedly, seeing its previous work, until completion.": "面向迭代开发的交互式自指 AI 循环，实现 Ralph Wiggum 技巧。Claude 反复处理同一任务、看到自己之前的成果，直到完成。",
    "Modernize legacy codebases (COBOL, legacy Java/C++, monolith web apps) with a structured assess / map / extract-rules / reimagine / transform / harden workflow and specialist review agents": "用结构化的 评估 / 梳理 / 提取规则 / 重构设想 / 转换 / 加固 工作流和专家评审 agent，现代化遗留代码库（COBOL、遗留 Java/C++、单体 Web 应用）",
    "Onboard a Code-with-Claude Makers Cardputer with one /maker-setup command — clones the build-with-claude repo, flashes UIFlow firmware, and installs the Claude Buddy app bundle.": "用一条 /maker-setup 命令完成 Code-with-Claude Makers Cardputer 的初始配置——克隆 build-with-claude 仓库、刷入 UIFlow 固件、安装 Claude Buddy 应用包。",
    "Security reminder hook that warns about potential security issues when editing files, including command injection, XSS, and unsafe code patterns": "在编辑文件时对潜在安全问题发出警告的安全提醒 hook，涵盖命令注入、XSS 和不安全的代码模式",
    "Skills for designing and building MCP servers that work seamlessly with Claude. Guides you through deployment models (remote HTTP, MCPB, local), tool design patterns, auth, and interactive MCP apps.": "用于设计和构建与 Claude 无缝协作的 MCP 服务器的 skill。带你走通部署模型（远程 HTTP、MCPB、本地）、工具设计模式、鉴权和交互式 MCP 应用。",
    "Solve competition math (IMO, Putnam, USAMO) with adversarial verification that catches what self-verification misses. Fresh-context verifiers attack proofs with specific failure patterns. Calibrated abstention over bluffing.": "用对抗式验证求解竞赛数学（IMO、Putnam、USAMO），捕捉自我验证遗漏的问题。全新上下文的验证者以特定失败模式攻击证明。宁可校准地弃权，也不虚张声势。",
    "Tools to maintain and improve CLAUDE.md files - audit quality, capture session learnings, and keep project memory current.": "维护和改进 CLAUDE.md 文件的工具——审计质量、沉淀会话经验、让项目记忆保持最新。",
    "Create new skills, improve existing skills, and measure skill performance. Use when users want to create a skill from scratch, update or optimize an existing skill, run evals to test a skill, or benchmark skill performance with variance analysis.": "创建新 skill、改进现有 skill、衡量 skill 表现。当用户想从零创建 skill、更新或优化现有 skill、跑 eval 测试 skill，或用方差分析对 skill 表现做基准测试时使用。",
    "Creates interactive HTML playgrounds — self-contained single-file explorers with visual controls, live preview, and prompt output with copy button. Includes templates for design playgrounds, data explorers, concept maps, and document critique.": "创建交互式 HTML 演练场——带可视化控件、实时预览、带复制按钮的提示输出的自包含单文件浏览器。内置设计演练场、数据浏览器、概念图和文档评析的模板。",
    "Pre-built small business workflows (including payroll planning, month-end close, weekly briefs, and growth campaigns) using your QuickBooks, PayPal, HubSpot, Docusign, Gsuite, O365, Canva, and other connected tools. You approve every step that touches money or customers.": "预置的小型企业工作流（含薪酬规划、月末结账、每周简报和增长活动），使用你已连接的 QuickBooks、PayPal、HubSpot、Docusign、Gsuite、O365、Canva 等工具。每一步涉及金钱或客户的操作都由你批准。",
    // 名字 partial 污染修复 + output style 名字
    "Bio Research": "生物研究",
    "Learning output style": "学习输出风格",
    "Explanatory output style": "解释式输出风格",
    // 磁贴/分屏操作 UI tooltip
    "Arrow keys move the tile. Perpendicular arrows preview a split; press Enter to commit or Escape to cancel.": "方向键移动图块。垂直方向的箭头预览拆分；按 Enter 确认，按 Esc 取消。",
    "Technology": "技术",
    "Travel": "旅游",
    ...
    "Research, code, and organize": "深度研究、编程、整理",
    ...
    "Build and prototype with Claude Design": "用 Claude Design 构建和制作原型",
  ...
  const dynamicPatterns = [
    ...
    [/^View, annotate, and sign PDFs in a live interactive viewer\.\s*Mark up contracts, fill forms with visual feedback,\s*(.*)$/i, "在实时交互式查看器中查看、批注、签署 PDF。标记合同、用视觉反馈填写表单、盖章批准、放置签名——然后下载批注后的副本。"],
    ...
    [/^Optimize business operations — vendor management, process documentation, change management,\s*(.*)$/i, "优化业务运营——供应商管理、流程文档、变更管理、产能规划、合规追踪。让你的组织高效运转。"],
    ...
    [/^Accelerate design workflows — critique, design system management, UX writing, accessibility audits,\s*(.*)$/i, "加速设计工作流——评审、设计系统管理、UX 写作、可访问性审计、研究综合、开发交接。从探索到像素级精准的规格。"],
    [/^Streamline people operations — recruiting, onboarding, performance reviews, compensation analysis,\s*(.*)$/i, "简化人事运营——招聘、入职、绩效评估、薪酬分析、政策指导。保持合规，让团队顺畅运转。"],
    [/^Streamline engineering workflows — standups, code review, architecture decisions, incident response,\s*(.*)$/i, "简化工程工作流——站会、代码评审、架构决策、事故响应、技术文档。可与你现有的工具协同，也可独立使用。"],
    ...
    [/^Connect to preclinical research tools and databases \(literature search, genomics analysis, target prioritizatio\s*(.*)$/i, "连接临床前研究工具和数据库（文献检索、基因组分析、目标优先级排序），加速早期生命科学研发。"],
    [/^Prospect, craft outreach, and build deal strategy faster\.\s*Prep for calls, manage your pipeline, and\s*(.*)$/i, "更快地寻找潜在客户、设计触达、构建交易策略。准备通话、管理销售管道、撰写推动成交的个性化消息。"],
    [/^Speed up contract review, NDA triage, and compliance workflows for in-house legal teams\.\s*Draft legal briefs,\s*(.*)$/i, "加速企业内部法务团队的合同评审、NDA 分诊、合规工作流。起草法律简报、整理判例研究、管理机构知识。"],
    [/^Write feature specs, plan roadmaps, and synthesize user research faster\.\s*Keep stakeholders updated and\s*(.*)$/i, "更快地写功能规格、规划路线图、综合用户研究。让利益相关者随时了解进展，并保持领先于竞争格局。"],
    [/^Manage tasks, plan your day, and build up memory of important context about your work\.\s*Syncs with your\s*(.*)$/i, "管理任务、规划你的一天、积累工作中重要上下文的记忆。与你的日历、邮箱和聊天同步，让一切井然有序、按部就班。"],
    [/^Create content, plan campaigns, and analyze performance across marketing channels\.\s*Maintain brand voice\s*(.*)$/i, "跨营销渠道创建内容、规划活动、分析表现。保持品牌语调一致、追踪竞品、汇报哪些有效。"],
    [/^Streamline finance and accounting workflows, from journal entries and reconciliation to financial statement\s*(.*)$/i, "简化财务和会计工作流，从日记账、对账到财务报表和差异分析。加速审计准备、月末结账，让账目保持清晰。"],
    [/^Search across all of your company's tools in one place\.\s*Find anything across email, chat, documents, and wikis\s*(.*)$/i, "在一处搜索你公司所有工具。在邮件、聊天、文档和 wiki 之间查找任何内容，无需在应用间切换。"],
    [/^Search across all of your company’s tools in one place\.\s*Find anything across email, chat, documents, and wikis\s*(.*)$/i, "在一处搜索你公司所有工具。在邮件、聊天、文档和 wiki 之间查找任何内容，无需在应用间切换。"],
    [/^Write SQL, explore datasets, and generate insights faster\.\s*Build visualizations and dashboards, and turn raw\s*(.*)$/i, "更快地写 SQL、探索数据集、生成洞察。构建可视化和仪表盘，把原始数据变成给利益相关者的清晰故事。"],
    [/^Triage tickets, draft responses, escalate issues, and build your knowledge base\.\s*Research customer context\s*(.*)$/i, "分诊工单、起草回复、升级问题、构建知识库。调研客户上下文，把已解决的问题转化为自助服务内容。"],
    ...
    [/^New session in (.+)$/i, "在 $1 中新建会话"],
    [/^(\d[\d,]*) additions?, (\d[\d,]*) deletions?$/i, "$1 处新增，$2 处删除"],
