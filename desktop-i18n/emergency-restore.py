"""
desktop-i18n 紧急还原: 从 _backup/<version>/ byte-identical 还原 asar + claude.exe,
另外删 resources/zh-CN.json (i18n 独立的资源文件,跟 asar 解耦)。

跟 uninstall.bat 的区别:
- emergency 整个 asar + claude.exe 抹回备份, 会同时擦掉同时装着的其他 desktop-* 补丁
- uninstall 走 marker strip 只剥本档 marker + 删 zh-CN.json, 保留其他 desktop-* 补丁
...
def main():
    print("=" * 60)
    print("desktop-i18n · 紧急还原 (byte-identical)")
    print("=" * 60)
    print()
    print("WARNING: 这会把 asar 和 claude.exe 整个还原成原版,")
    print("         同时装着的其他补丁 (font / devtools) 会一起被擦掉。")
