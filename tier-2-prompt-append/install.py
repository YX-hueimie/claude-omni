"""
claude-omni · tier-2 · system prompt append

把本目录的 append.txt 内容写进 Claude Desktop 的 SDK system prompt 主体末尾
(成为 system prompt 的一部分, system 角色)。
Code tab 走 appendSystemPrompt 字段 (SDK 默认 system prompt 保留, 本档内容拼在末尾)。
Cowork 走 baseSystemPrompt prepend (本档内容 prepend 到原 P 之前, 不剥三段)。

注: tier-2 用的 append.txt 是精简版 deployment-context 措辞 — 只含
    Deployment context / Defaults / Response style 三段, 因为 SDK 默认那大段
    还在前面, 不重复。tier-5 那份 append.txt 是整段替换 SDK 默认用的完整版
    (含模拟整个 SDK 默认结构), 跟本 tier 不是同一份, 只是同名。

    PERSONA-INJECT 格式 (tier-1 用的) 是为 user 角色设计的; 塞到 system 角色下
    会触发对抗输入识别, 拒绝率反升, 所以本 tier 不用那种措辞。
...
def main():
    ...
    # tier-2: 读本目录的 append.txt (deployment-context 措辞, 适合 system 角色)
