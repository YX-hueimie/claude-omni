"""
tier-5 卸载: 解 asar -> 剥自己 marker -> 重打包 -> 写回。
...
def main():
    ...
    if not has_l2:
        ...
        print()
        print("=" * 60)
        print("✓ tier-5 uninstall 完成 (asar 无需改动)")
        print("=" * 60)
        input("按回车退出...")
        return
    ...
    if not other_present and backup_complete:
        ...
        # 扫 .vite/build/*.js 找含 PATCH_START marker 的那一个/几个, 全部剥。
        build_dir = extracted / ".vite" / "build"
        if not build_dir.exists():
            raise SystemExit(f"  ! 找不到 {build_dir}")
        js_files = [f for f in sorted(build_dir.glob("*.js"))]
        patched_files = []
        for jf in js_files:
            try:
        ...
        for jf, text in patched_files:
            ...
            # Sanity check: 剥 marker 后若产生 "systemPrompt:," 空字段值 (字段值替换式注入的
            # 副作用), 重打包写回会让 Claude 启动 parse 失败。检测到 → 中止 uninstall, 提示
            # emergency-restore (字节级还原, 不依赖剥 marker)。当前 install.py 用重复 key 覆盖
            # 策略, 不触发这条路径; 保留作为兼容性兜底。
            if re.search(r'systemPrompt:\s*[,)\]}]', new_text):
                for p in [extracted, new_unpacked]:
                    if p.exists():
                        shutil.rmtree(p, ignore_errors=True)
                print(f"  ! 剥 marker 后 {jf.name} 检测到 `systemPrompt:` 后空字段值")
